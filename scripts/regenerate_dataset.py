...
def main(args):
    ...
    metainfo_json_dict = {}
    metainfo_json_out_path = f"/n/holylabs/ydu_lab/Lab/zhangxiangcheng/code/SAILOR/scratch_dir/libero_cache/{args.libero_task_suite}_task{args.task_id}_metainfo.json"
    ...
    for i in tqdm.tqdm(range(len(orig_data.keys()))):
        ...
        for _, action in tqdm.tqdm(enumerate(orig_actions)):
...
if __name__ == "__main__":
    ...
    task_indices = parse_task_indices(args.task_id)
    if len(task_indices) > 1:
        ...
        for i in range(len(task_indices)):
            ...
            cur_args.task_id = task_indices[i]
